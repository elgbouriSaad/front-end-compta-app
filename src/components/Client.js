...
export default function Client() {
  ...
  useEffect(() => {
    if (id) {
      const fetchClient = async () => {
        try {
          const data = await makeApiGetRequest(`/api/v1/clients/${id}`);
          setFormData(data);
        } catch (error) {
  ...
  const handleFormSubmit = async (formData) => {
    try {
      if (id) {
        await makeApiPutRequest(`/api/v1/clients/${id}`, formData);
      } else {
        await makeApiPostRequest("/api/v1/clients", formData);
      }
      navigate("/");
    } catch (error) {
  ...
  return (
    <div className="container">
      <div className="card">
      <h3 className="card-header">{id && !isEditMode ? t("client.details") : (id ? t("client.edit") : t("client.title"))}</h3>
