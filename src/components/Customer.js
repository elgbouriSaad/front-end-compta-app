...
import { fetchAccountantIds } from "./GetUtils";
export default function Customer() {
  ...
  const [accountantIds, setAccountantIds] = useState([]);
  ...
  useEffect(() => {
    const fetchIds = async () => {
      const ids = await fetchAccountantIds();
      setAccountantIds(ids);
    };
    fetchIds();
  }, []);
  ...
  useEffect(() => {
    if (id) {
      const fetchCustomer = async () => {
        try {
          const data = await makeApiGetRequest(`/api/v1/customers/${id}`);
          setFormData(data);
        } catch (error) {
  ...
  const handleFormSubmit = async (formData) => {
    try {
      if (id) {
        await makeApiPutRequest(`/api/v1/customers/${id}`, formData);
      } else {
        await makeApiPostRequest("/api/v1/customers", formData);
      }
      navigate("/");
    } catch (error) {
  ...
  const handleChange = (event) => {
    ...
    setFormData((prevFormData) => ({
      ...prevFormData,
      accountantId: accountantIds.length > 0 ? accountantIds[0] : "",
      [id]: value,
    }));
  };
  ...
  return (
    <div className="container">
      <div className="card">
      <h3 className="card-header">{id && !isEditMode ? t("customer.details") : (id ? t("customer.edit") : t("customer.title"))}</h3>
