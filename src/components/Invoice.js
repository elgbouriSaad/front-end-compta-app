...
export default function Invoice() {
  ...
  useEffect(() => {
    if (id) {
      const fetchInvoice = async () => {
        try {
          const data = await makeApiGetRequest(`/api/v1/invoices/${id}`);
          setFormData({
            paymentDelay: data.paymentDelay,
            clientId: data.clientId,
            productQuantities: data.invoiceProducts.map((product) => ({
              productId: product.productId,
              quantity: product.quantity,
            })),
          });

          const mappedProducts = data.invoiceProducts.map((product) => ({
            productId: product.productId,
            quantity: product.quantity,
            label: product.label,
          }));
  ...
  const handleFormSubmit = async (formData) => {
    try {
      if (id) {
        await makeApiPutRequest(`/api/v1/invoices/update/${id}`, formData);
      } else {
        await makeApiPostRequest("/api/v1/invoices", formData);
      }
  ...
  return (
    <div className="container">
      <div className="">
        <div className="card">
        <h3 className="card-header">{id && !isEditMode ? t("invoice.details") : (id ? t("invoice.edit") : t("invoice.title"))}</h3>
